...
class ZeroWeightAgent(bustersAgents.BustersAgent):
    ...
    def checkReset(self, newBeliefs, oldBeliefs):
        for i in range(len(newBeliefs)):
            newKeys = [x for x in list(newBeliefs[i].keys()) if newBeliefs[i][x] != 0]
            oldKeys = [x for x in list(oldBeliefs[i].keys()) if oldBeliefs[i][x] != 0]
...
class DoubleInferenceAgent(bustersAgents.BustersAgent):
    ...
    def distCompare(self, dist, refDist):
        ...
        for key in set(list(refDist.keys()) + list(dist.keys())):
            if not key in list(dist.keys()):
                dist[key] = 0.0
            if not key in list(refDist.keys()):
                refDist[key] = 0.0
        # calculate l2 difference
        l2 = 0
        for k in list(refDist.keys()):
        ...
        if l2 > self.L2Tolerance:
            if self.errors == 0:
                ...
                detail = '\n'.join(['%9s:%5s%-25s%-25s' % (x, '', dist[x], refDist[x]) for x in set(list(dist.keys()) + list(refDist.keys()))])
